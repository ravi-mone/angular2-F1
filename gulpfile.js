...
gulp.task('build.assets.dev', ['build.js.dev'], function () {
  return gulp.src(['./app/**/*.html', './app/**/*.css', './app/**/*.jpg', './app/**/*.ico', './app/**/fonts/*'])
    .pipe(gulp.dest(PATH.dest.dev.all));
});
...
function injectableDevAssetsRef() {
  ...
  var css = PATH.src.css.map(function(path) {
    return join(PATH.dest.dev.css, path.split('/').pop());
  });
  src.push(PATH.dest.dev.ng2, PATH.dest.dev.router,
           join(PATH.dest.dev.all, '**/*.css'));
  return src.concat(css);
}
